  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-20 flex items-center justify-between p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <button
          onClick={prevSlide}
          className="bg-black/50 hover:bg-[#00AEEF] text-white w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md transition-all active:scale-95 border border-white/20"
        ...
        {images.map((_, index) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-all ${
              index === currentIndex ? 'bg-[#00AEEF] scale-125' : 'bg-white/50 hover:bg-white/80'
            }`}
